...
proposer_probs = 26/10000
block_proposers_size = 26
...
class AlgoNode(object):
	...
	def __init__(self, env, id,  honest, manual_committee_size=5):
		self.env = env
		self.id = id
	...
	def FixedGenerator(self):
		...
		while flag:
			...
			if not self.Tenative and not self.waiting_reply:
				...
				empty_block = self.generate_empty_block(current_round)
				proposer = self.Sortition(self.sk, current_round, block_proposer, block_proposers_size)

				if proposer:

					self.propose_block(proposer, current_round)
					print(f'node {self.id} selected as proposer with toekns {self.tokens} and chain total {self.chain.total_tokens}')
					yield self.env.timeout(2)


				else:
					print(f'node {self.id} failed to sort as proposer with toknes {self.tokens}')
			...
			# 	print(f'node {self.id} voters info {self.voters}')
			# 	break;
			# 	yield self.env.timeout(1)
	...
	def Committe_vote(self, ctx, round, step, threshold, value):
		...
		if sort_result:
			...
			print(f'node {self.id} failed to enter committee at step {step} in round {round} and step {step}')
			...
			if key in self.vote_buffer.keys():
				...
				for hash in self.vote_buffer[key]:
					if self.vote_buffer[key][hash]>=thresh_vote:
						...
						print(f'failed at counting votes with votes info {self.vote_buffer[key][hash]} and committ size {self.manual_committee_size}')
						print(f'recieved voters are {self.voters[key]}')
